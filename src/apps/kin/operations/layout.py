...
from apps.kin import consts, helpers
from apps.kin.layout import format_amount, require_confirm, split, trim_to_rows, ui
...
async def confirm_allow_trust_op(ctx, op: KinAllowTrustOp):
    if op.is_authorized:
        t = "Allow Trust"
    else:
        t = "Revoke Trust"
...
async def confirm_bump_sequence_op(ctx, op: KinBumpSequenceOp):
    text = Text("Confirm operation", ui.ICON_CONFIRM, icon_color=ui.GREEN)
...
async def confirm_change_trust_op(ctx, op: KinChangeTrustOp):
    if op.limit == 0:
        t = "Delete Trust"
    else:
        t = "Add Trust"
...
async def confirm_create_account_op(ctx, op: KinCreateAccountOp):
    text = Text("Confirm operation", ui.ICON_CONFIRM, icon_color=ui.GREEN)
...
async def confirm_create_passive_offer_op(ctx, op: KinCreatePassiveOfferOp):
    if op.amount == 0:
...
async def confirm_manage_offer_op(ctx, op: KinManageOfferOp):
    if op.offer_id == 0:
        text = "New Offer"
    else:
        if op.amount == 0:
...
async def confirm_manage_data_op(ctx, op: KinManageDataOp):
...
async def confirm_path_payment_op(ctx, op: KinPathPaymentOp):
    text = Text("Confirm operation", ui.ICON_CONFIRM, icon_color=ui.GREEN)
...
async def confirm_payment_op(ctx, op: KinPaymentOp):
    text = Text("Confirm operation", ui.ICON_CONFIRM, icon_color=ui.GREEN)
...
async def confirm_set_options_op(ctx, op: KinSetOptionsOp):
    ...
    if op.signer_type is not None:
        ...
        if op.signer_type == consts.SIGN_TYPE_ACCOUNT:
            ...
            raise ProcessError("Kin: invalid signer type")


def _format_thresholds(op: KinSetOptionsOp) -> tuple:
    text = ()
...
def _format_flags(flags: int) -> tuple:
    if flags > consts.FLAGS_MAX_SIZE:
        raise ProcessError("Kin: invalid flags")
    text = ()
...
async def confirm_asset_issuer(ctx, asset: KinAssetType):
    if asset is None or asset.type == consts.ASSET_TYPE_NATIVE:
        return
