...
from trezor.messages.WanchainSignTx import WanchainSignTx
...
async def wanchain_sign_tx(ctx, msg):
    from trezor.crypto.hashlib import sha3_256
    ...
    total_length = wanchain_get_total_length(msg, data_total)

    sha = HashWriter(sha3_256)
    sha.extend(rlp.encode_length(total_length, True))  # total length

    for field in [msg.txtype, msg.nonce, msg.gas_price, msg.gas_limit, msg.to, msg.value]:
        sha.extend(rlp.encode(field))
    ...
    digest = sha.get_digest(True)  # True -> use keccak mode
    return await wanchain_send_signature(ctx, msg, digest)
...
def wanchain_get_total_length(msg: WanchainSignTx, data_total: int) -> int:
    length = 0
    for field in [msg.txtype, msg.nonce, msg.gas_price, msg.gas_limit, msg.to, msg.value]:
        length += rlp.field_length(len(field), field[:1])
...
async def wanchain_send_signature(ctx, msg: WanchainSignTx, digest):
    from trezor.crypto.curve import secp256k1
    from ..common import seed
