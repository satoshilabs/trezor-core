...
static int onegram_is_canonical(uint8_t v, uint8_t signature[64])
{
    (void)v;
...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_sign(size_t n_args,
  ...
  switch (canonical) {
    case CANONICAL_SIG_ETHEREUM:
      ...
      break;
    case CANONICAL_SIG_ONEGRAM:
      is_canonical = onegram_is_canonical;
      break;
  }
  if (sk.len != 32) {
    mp_raise_ValueError("Invalid length of secret key");
...
STATIC const mp_rom_map_elem_t mod_trezorcrypto_secp256k1_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_CANONICAL_SIG_ONEGRAM),
     MP_OBJ_NEW_SMALL_INT(CANONICAL_SIG_ONEGRAM) },
};
